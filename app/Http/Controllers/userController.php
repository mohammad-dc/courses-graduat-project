...
class UserController extends Controller
{
    ...
    {
        $query = DB::select(
            "select id, full_name, email, password username, univ_number, gender from users where username='{$req->username}' and password='{$req->password}'"
        );
    ...
    public function editProfile(Request $req)
    {
        if ($req->session()->has('user')) {
            ...
            $query = DB::select(
                "update users set full_name='{$req->full_name}', email='{$req->email}', password='{$req->password}', username='{$req->username}', univ_number='{$req->univ_number}', gender='{$req->gender}' where id={$user->id}"
            );
    ...
    public function viewProfile(Request $req)
    {
        if ($req->session()->has('user')) {
            ...
            return view('profile', ['user' => $query, 'account' => $user]);
        } else {
            return redirect('login?error=true');
        }
    }
